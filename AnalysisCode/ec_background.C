...
// Create histos
   TH2F *hi_ecal_occ[3];
   TH2F *hi_ecal_occ_cut[3]; 
   TH1F *hi_ecal_vz_all[3];
   TH1F *hi_ecal_vz_e[3];
   TH1F *hi_ecal_vz_g[3];
   TH1F *hi_ecal_vz_h[3];
   TH1F *hi_ecal_vz_n[3];
   TH2F *hi_ecal_origin_all[3]; 
   TH1F *hi_ecal_edep[3]; 

   for(int i=0; i<3; i++) {
       int nstrip=68;
       if(i>0) nstrip=36;
       hi_ecal_occ[i] = new TH2F(Form("hi_ecal_occ %d",i+1), Form("hi_ecal_occ %d",i+1),nstrip, 1.,nstrip*1.+1.,3,1.,4.);
       hi_ecal_occ[i]->GetXaxis()->SetTitle("Strip");
       hi_ecal_occ[i]->GetYaxis()->SetTitle("View");
       hi_ecal_occ_cut[i] = new TH2F(Form("hi_ecal_occ_cut %d",i+1), Form("hi_ecal_occ_cut %d",i+1),nstrip, 1.,nstrip*1.+1.,3,1.,4.);
       hi_ecal_occ_cut[i]->GetXaxis()->SetTitle("Strip");
       hi_ecal_occ_cut[i]->GetYaxis()->SetTitle("View");
       hi_ecal_vz_all[i] = new TH1F(Form("hi_ecal_vz_all %d",i+1), Form("hi_ecal_vz_all %d",i+1),300, 0., 900.);
       hi_ecal_vz_all[i]->GetXaxis()->SetTitle("Z_{vertex} (cm)");
       hi_ecal_vz_all[i]->GetYaxis()->SetTitle("Rate (MHz/3cm)");
       hi_ecal_vz_e[i] = new TH1F(Form("hi_ecal_vz_e %d",i+1), Form("hi_ecal_vz_e %d",i+1), 300, 0., 900.);
       hi_ecal_vz_e[i]->GetXaxis()->SetTitle("Z_{vertex} (cm)");
       hi_ecal_vz_e[i]->GetYaxis()->SetTitle("Rate (MHz/3cm)");
       hi_ecal_vz_g[i] = new TH1F(Form("hi_ecal_vz_g %d",i+1), Form("hi_ecal_vz_g %d",i+1),300, 0., 900.);
       hi_ecal_vz_g[i]->GetXaxis()->SetTitle("Z_{vertex} (cm)");
       hi_ecal_vz_g[i]->GetYaxis()->SetTitle("Rate (MHz/3cm)");
       hi_ecal_vz_h[i] = new TH1F(Form("hi_ecal_vz_h %d",i+1), Form("hi_ecal_vz_h %d",i+1),300, 0., 900.);
       hi_ecal_vz_h[i]->GetXaxis()->SetTitle("Z_{vertex} (cm)");
       hi_ecal_vz_h[i]->GetYaxis()->SetTitle("Rate (MHz/3cm)");
       hi_ecal_vz_n[i] = new TH1F(Form("hi_ecal_vz_n %d",i+1), Form("hi_ecal_vz_n %d",i+1),300, 0., 900.);
       hi_ecal_vz_n[i]->GetXaxis()->SetTitle("Z_{vertex} (cm)");
       hi_ecal_vz_n[i]->GetYaxis()->SetTitle("Rate (MHz/3cm)");
       hi_ecal_origin_all[i] = new TH2F(Form("hi_ecal_origin_all %d",i+1), Form("hi_ecal_origin_all %d",i+1),300, 0., 900.,200, 0., 600.);
       hi_ecal_origin_all[i]->GetXaxis()->SetTitle("Z_{vertex} (cm)");
       hi_ecal_origin_all[i]->GetYaxis()->SetTitle("R_{vertex} (cm)");
       hi_ecal_edep[i] = new TH1F(Form("hi_ecal_edep %d",i+1), Form("hi_ecal_edep %d",i+1),200, 0.,10.);
       hi_ecal_edep[i]->GetXaxis()->SetTitle("E(MeV)");
       hi_ecal_edep[i]->GetYaxis()->SetTitle("Rate (MHz/50keV)");
   }
...
// in luminosity mode (50*2300 or 118600 electrons in 250ns window)
   ...
   for(int i=0; i<3; i++) {
       hi_ecal_occ[i]->Scale(1000/time/6);
       hi_ecal_occ_cut[i]->Scale(1000/time/6);
       hi_ecal_vz_all[i]->Scale(1/time);
       hi_ecal_vz_e[i]->Scale(1/time);
       hi_ecal_vz_g[i]->Scale(1/time);
       hi_ecal_vz_h[i]->Scale(1/time);
       hi_ecal_vz_n[i]->Scale(1/time);
       hi_ecal_origin_all[i]->Scale(1/time);
       hi_ecal_edep[i]->Scale(1/time);
   }
   ...
   for(int i=1; i<3; i++) {
	   ...
	   for(int is=0; is<hi_ecal_occ[i]->GetNbinsX(); is++) {
	     int layer = i*3+iv+1;
	     int strip = is +1;
	     float value = hi_ecal_occ[i]->GetBinContent(is+1,iv+1);
	     fprintf(fp,"%d\t%d\t%8.1f\n",layer,strip,value);
	   }
       }
   }
   ...
   for(int i=1; i<3; i++) {
	   ...
	   for(int is=0; is<hi_ecal_occ_cut[i]->GetNbinsX(); is++) {
	     int layer = i*3+iv+1;
	     int strip = is +1;
	     float value = hi_ecal_occ_cut[i]->GetBinContent(is+1,iv+1);
	     fprintf(fp,"%d\t%d\t%8.1f\n",layer,strip,value);
	   }
       }
   }
   ...
   c_vz->Divide(1,2);
   c_vz->cd(1);
   hi_ecal_vz_all[1]->SetMinimum(0.001);
   hi_ecal_vz_all[1]->Draw("H");
   hi_ecal_vz_e[1]->SetLineColor(2);
   hi_ecal_vz_e[1]->Draw("SAME");
   hi_ecal_vz_g[1]->SetLineColor(4);
   hi_ecal_vz_g[1]->Draw("SAME");
   hi_ecal_vz_h[1]->SetLineColor(kGreen);
   hi_ecal_vz_h[1]->Draw("SAME");
   hi_ecal_vz_n[1]->SetLineColor(kGreen+2);
   hi_ecal_vz_n[1]->Draw("SAME");
   TLegend *leg1 = new TLegend(0.7,0.75,0.96,0.96);
   leg1->SetTextSize(.04);
   leg1->AddEntry(hi_ecal_vz_all[1],"All","l");
   leg1->AddEntry(hi_ecal_vz_e[1],"electrons","l");
   leg1->AddEntry(hi_ecal_vz_g[1],"photons","l");
   leg1->AddEntry(hi_ecal_vz_n[1],"neutron","l");
   leg1->AddEntry(hi_ecal_vz_h[1],"hadron","l");
   leg1->Draw();
   c_vz->cd(2);
   hi_ecal_vz_all[2]->SetMinimum(0.001);
   hi_ecal_vz_all[2]->Draw("H");
   hi_ecal_vz_e[2]->SetLineColor(2);
   hi_ecal_vz_e[2]->Draw("SAME");
   hi_ecal_vz_g[2]->SetLineColor(4);
   hi_ecal_vz_g[2]->Draw("SAME");
   hi_ecal_vz_h[2]->SetLineColor(kGreen);
   hi_ecal_vz_h[2]->Draw("SAME");
   hi_ecal_vz_n[2]->SetLineColor(kGreen+2);
   hi_ecal_vz_n[2]->Draw("SAME");
   TLegend *leg2 = new TLegend(0.7,0.75,0.96,0.96);
   leg2->SetTextSize(.04);
   leg2->AddEntry(hi_ecal_vz_all[2],"All","l");
   leg2->AddEntry(hi_ecal_vz_e[2],"electrons","l");
   leg2->AddEntry(hi_ecal_vz_g[2],"photons","l");
   leg2->AddEntry(hi_ecal_vz_n[2],"neutron","l");
   leg2->AddEntry(hi_ecal_vz_h[2],"hadron","l");
   leg2->Draw();

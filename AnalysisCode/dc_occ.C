...
*/
    ...
    TLine *rga_reg1 = new TLine(0.5,2.59,6.5,2.59);
    rga_reg1->SetLineColor(kBlue+2);
    rga_reg1->SetLineStyle(2);
    rga_reg1->Draw("SAMEPE");
    ...
    TLine *rga_reg2 = new TLine(0.5,0.76,6.5,0.76);
    rga_reg2->SetLineColor(kRed+2);
    rga_reg2->SetLineStyle(2);
    rga_reg2->Draw("SAMEPE");
    ...
    TLine *rga_reg3 = new TLine(0.5,1.18,6.5,1.18);
    rga_reg3->SetLineColor(kGreen+2);
    rga_reg3->SetLineStyle(2);
    rga_reg3->Draw("SAMEPE");
    mypol0[2]->SetLineColor(kGreen+2);
    hi_dcocc_region[2]->Fit("mypol2","SAME");
    TLegend *l_occ=new TLegend(0.5,0.75,0.96,0.96);
    l_occ->SetHeader("Transverse: Luminosity = 10^33; RGA: Luminosity = 10^35","C");
    for(int i=0; i<3; i++) {
      l_occ->AddEntry(hi_dcocc_region[i],Form("Transverse region %i occupancy: %.2f %%",i+1,mypol0[i]->GetParameter(0)),"p");
    }
    l_occ->AddEntry(rga_reg1,"RGA region 1 occupancy: 2.59%","l");
    l_occ->AddEntry(rga_reg2,"RGA region 2 occupancy: 0.76%","l");
    l_occ->AddEntry(rga_reg3,"RGA region 3 occupancy: 1.18%","l");
